...
install(Context) ->
    z_db:q("
        create table if not exists mediarunner_consumer_totals (
            owner_id integer primary key,
            submitted bigint not null,
            completed bigint not null,
            failed bigint not null,
            cache_hits bigint not null,
            processing_seconds bigint not null,
            callback_failures bigint not null,
            last_job bigint not null
        )", Context),
    ok.
...
archive(Before, Context) ->
    z_db:q("
        with removed as (
            delete from mediarunner_job
            where created < $1
              and payload is null
            returning *
        )
        insert into mediarunner_consumer_totals
            (owner_id, submitted, completed, failed, cache_hits,
             processing_seconds, callback_failures, last_job)
        " ++ totals("removed") ++ "
        on conflict (owner_id) do update set
            ...
            last_job = greatest(mediarunner_consumer_totals.last_job, excluded.last_job)
        ", [Before], Context),
    ok.
...
totals(Table) ->
    "
        select
            ...
            coalesce(sum(
                case when not cache_hit and started is not null and finished is not null
                    then greatest(0, finished - started)
                    else 0
                end
            ), 0)::bigint as processing_seconds,
    ...
    {ok, Rows} = z_db:qmap("
        ...
        totals as (
            select
                ...
                sum(processing_seconds)::bigint as processing_seconds,
                sum(callback_failures)::bigint as callback_failures,
                max(last_job) as last_job
            from history
            group by owner_id
        ),
        queue as (
            select
                owner_id,
                count(*) filter (where status in ('queued', 'starting')) as queued,
        ...
        cache as (
            select owner_id, count(*) as cached_files, sum(size)::bigint as cached_bytes
        ...
        owners as (
            ...
            union select owner_id from cache
        )
        select
            owner_id,
            ...
            coalesce(callback_failures, 0) as callback_failures,
            last_job,
            coalesce(queued, 0) as queued,
            coalesce(running, 0) as running,
            coalesce(callbacks_pending, 0) as callbacks_pending,
            coalesce(cached_files, 0) as cached_files,
            coalesce(cached_bytes, 0) as cached_bytes
        from owners
        left join totals using (owner_id)
        left join queue using (owner_id)
        left join cache using (owner_id)", Context),
    maps:from_list([
        {maps:get(<<"owner_id">>, Row), maps:remove(<<"owner_id">>, Row)}
        || Row <- Rows
    ]).
