...
install(Context) ->
    ...
    z_db:q("create index if not exists mediarunner_job_delivery on mediarunner_job(delivery, next_attempt)", Context),
    z_db:q("alter table mediarunner_job add column if not exists cache_hit boolean not null default false", Context),
...
install_cache(Context) ->
    ...
    z_db:q("create index if not exists mediarunner_job_file_hash on mediarunner_job_file(owner_id,hash)", Context),
    z_db:flush(Context),
    ok.
...
recover(Context) ->
    ...
    z_db:q("
        update mediarunner_job
        set status='queued', started=null
    ...
    z_db:q("
        ...
        where delivery='sending'", Context),
    ok.
...
next(deliver, Context) ->
    case
        z_db:qmap("
            update mediarunner_job
            set delivery='sending', attempts=attempts+1
            where id=(
                select id
                from mediarunner_job
...
queue_usage(Context) ->
    [{Count, Bytes, Active}] = z_db:q("
        select
            count(*),
            coalesce(sum(octet_length(payload) + coalesce(octet_length(result),0)),0),
...
started(Id, Context) ->
    z_db:q("
        update mediarunner_job
        set status='running',started=$2
        where id=$1", [Id, now()], Context
    ),
    ok.
...
result(Id, Result, Context) ->
    result(Id, Result, false, Context).
...
delivered(Id, _, Context) ->
    ...
    case Attempts >= 12 orelse now() > Expires of
        ...
        false ->
            ...
            z_db:q("
                update mediarunner_job
                set delivery='pending',next_attempt=$2
                where id=$1",
                [Id, now() + Delay],
                Context
            ),
...
finish_delivery(Id, Status, Context) ->
    z_db:q("
        update mediarunner_job
        set delivery=$2,payload=null,result=null, callback_token=null,callback_url=null,delivered_at=$3
        where id=$1",
        [Id, Status, now()],
        Context
    ),
...
snapshot(Filter, Context) ->
    {ok, Counts} = z_db:qmap("
        select case when status='starting' then 'queued' else status end as status,count(*) as count
        from mediarunner_job
        where finished >= $1
    ...
    {ok, Delivery} = z_db:qmap("
        select delivery,count(*) as count
        from mediarunner_job
        where delivered_at >= $1
          or delivery in ('waiting','pending','sending')
        group by delivery",
        [now() - 86400],
        Context
    ),
    {ok, Hourly} = z_db:qmap("
        select (finished/3600)*3600 as hour, count(*) filter (where status='completed') as completed, count(*) filter (where status='failed') as failed
        from mediarunner_job
        where finished >= $1
        group by hour
        order by hour",
    ...
    {ok, Jobs} = z_db:qmap("
        ...
        from mediarunner_job
        where ($1 = '' or status=$1 or delivery=$1 or ($1='queued' and status='starting'))
        order by case when status in ('queued','starting','running') then 0 when delivery in ('pending','sending','failed') then 1 else 2 end, created desc
        limit 100",
        [Filter],
        Context
    ),
...
capacity(Context) ->
    case m_site:get(mediarunner_capacity, Context) of
        undefined ->
            mediarunner_capacity:snapshot(Context);
        V ->
            V
    end.
