...
-export([m_get/3, m_post/3, authorize/1, validate/2]).
...
m_post(Path, #{payload := Payload}, Context) when is_map(Payload) ->
    case authorize(Context) of
        ok -> control(Path, Payload, Context);
        {error, _} = Error -> Error
    end;
m_post(_, _, _) -> {error, payload}.
    ...
    case validate(Job, m_site:get(mediarunner_callback_urls, Context)) of
        ok ->
            case mediarunner_queue:submit(Job, z_acl:user(Context), Context) of
                ...
                {error, {missing, Hashes}} -> {ok, #{outcome => missing, missing => Hashes}};
                {error, Reason} -> outcome(Reason)
            end;
        {error, _} -> outcome(invalid_job)
    end;
    ...
    case valid_hash(Hash) andalso Size =< z_media_runner_protocol:input_limit() of
        true ->
            case mediarunner_queue:upload({reserve, Hash, Size}, z_acl:user(Context), Context) of
                {ok, present} -> outcome(present);
                {ok, Token} -> {ok, #{outcome => upload, upload_token => Token}};
                {error, conflict} -> outcome(busy);
                {error, Reason} -> outcome(Reason)
            end;
        false -> {error, payload}
    end;
...
control(_, _, _) -> {error, unknown_path}.

outcome(Outcome) -> {ok, #{outcome => Outcome}}.
